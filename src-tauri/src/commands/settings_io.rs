...
/// Export all `app_config` entries to a JSON file.
#[tauri::command]
pub fn export_settings_to_file(db: State<'_, Arc<Database>>, path: String) -> Result<(), String> {
    let json = serialize_settings(&db)?;
    std::fs::write(Path::new(&path), json).map_err(|e| format!("Write error: {}", e))
}

/// Import settings from a JSON file, overwriting matching `app_config` entries.
#[tauri::command]
pub fn import_settings_from_file(db: State<'_, Arc<Database>>, path: String) -> Result<(), String> {
    ...
    deserialize_settings(&db, &json)
}

fn serialize_settings(db: &Arc<Database>) -> Result<String, String> {
    let conn = db.conn();
    let mut stmt = conn
    ...
    for (key, value) in entries {
        // Round-trip the value through JSON if it parses; otherwise keep as string.
...
fn deserialize_settings(db: &Arc<Database>, json: &str) -> Result<(), String> {
